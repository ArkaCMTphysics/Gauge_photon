...
for i, pc in enumerate(p_centers):
    ...
    edge_col = CB["orange"] if is_center else CB["blue"]
    axe.add_patch(Ellipse((0,pc), width=1.0, height=0.62,
                           facecolor=(*blue_rgb, fill_alpha), edgecolor=edge_col, linewidth=2.0))
    axe.add_patch(Circle((0,pc), radius=0.055, facecolor="white", edgecolor=edge_col, linewidth=1.3, zorder=5))
    if i==0:
